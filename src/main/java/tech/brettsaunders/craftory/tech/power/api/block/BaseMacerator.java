...
public class BaseMacerator extends BaseOneToOneMachine{
  ...
  protected void processComplete() {
    ...
    boolean isOre = currentRecipe.getX().substring(currentRecipe.getX().length()-3).equalsIgnoreCase("ore");
    if (outputSlots.get(0) == null || outputSlots.get(0).getType() == Material.AIR) {
      ItemStack stack = currentProduct.clone();
      if(isOre) stack.setAmount(2);
      outputSlots.set(0, stack);
    } else {
      if(isOre) outputSlots.get(0).setAmount(outputSlots.get(0).getAmount() + 2);
      else outputSlots.get(0).setAmount(outputSlots.get(0).getAmount() + 1);
    }
